...
So the output will be the confusion matrix like below. This output gives us, the confusion matrix of the first iteration and the last iteration and the gain/loss percentage. In addition to that, you are going to obtain the figure of each accuracy measures, which is calculated for each iterative steps.
